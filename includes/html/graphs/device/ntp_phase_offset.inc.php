 ...
 * are -- those RRD files genuinely don't have a `phase_offset`
 ...
 * field is based on -- a genuinely different quantity from the
 * existing `offset` dataset, not a duplicate.
 */
...
$rrd_options[] = 'COMMENT:Phase Offset (s)      Now      Min      Max\\n';
$rrd_additions = '';
...
foreach ($components as $array) {
    ...
    if (Rrd::checkRrdExists($rrd_filename)) {
        ...
        $rrd_options[] = 'DEF:DS' . $count . '=' . $rrd_filename . ':phase_offset:AVERAGE';
        $rrd_options[] = 'LINE1.25:DS' . $count . '#' . $color . ':' . str_pad(substr((string) $array['peer'], 0, 15), 15) . $stack;
        $rrd_options[] = 'GPRINT:DS' . $count . ':LAST:%7.6lf';
        $rrd_options[] = 'GPRINT:DS' . $count . ':MIN:%7.6lf';
        $rrd_options[] = 'GPRINT:DS' . $count . ':MAX:%7.6lf\\l';
        $count++;
    }
}
